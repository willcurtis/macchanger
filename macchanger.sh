...
# Modernised by:   community contributors
#
...
VERSION="1.0.0"
AUTHOR="Original: shilch; Updates: community contributors"
YEAR="2016-2025"
...
ensure_device_exists() {
  ...
  if ! ifconfig "${dev}" >/dev/null 2>&1; then
    die "Unable to find device/interface '${dev}'. Is it down?"
  fi
}
...
get_type() {
  local dev=$1
  # 'type:' only appears on some interfaces; fall back gracefully.
...
generate_mac() {
  ...
  printf '%s:%s:%s:%s:%s:%s'     "${first_hex}" "${bytes:2:2}" "${bytes:4:2}" "${bytes:6:2}" "${bytes:8:2}" "${bytes:10:2}"
}

lookup_vendor() {
  # Best-effort OUI lookup. Uses a local text OUI database if available.
  # You can override the path via MACCHANGER_OUI_PATH.
  local mac=$1
  local oui_file
  oui_file=${MACCHANGER_OUI_PATH:-}

  if [[ -z "${oui_file}" ]]; then
    # Common locations (Wireshark / IEEE)
    local candidates=(
      "~/.cache/oui.txt"
    )
  ...
  # Try a few common OUI formats
  local vendor
  vendor=$(grep -i -m1 -E "^(?:${prefix}[[:space:]]|${prefix}[[:tspace:]]|${prefix}[[:space:]])" "${oui_file}" 2>/dev/null | sed -E 's/^[^[:space:]]+[[:space:]]+//')
